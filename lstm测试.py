...
def pre_data(data_path):
    ...
    data = data.drop(columns=data.columns[:2])  # 不要前3列
...
y = np.empty((1,1))
csv_files = glob.glob('D:\测井\数据\A_gs地区数据\\Ags119_wire-cg.csv')
for file in csv_files:
    data,data1 = pre_data(file)
    data = fix_data(data)
    if data.empty:
        continue
    ...
    print(label.shape)
    feature,scaler_x = normalization(feature)
    label,scaler_y = normalization(label)
...
# delta = lenth1-lenth2
# data1 = data1[delta:]
# assert len(data1) == lenth2
# data1["DEN_predict"] = y_pred
# data1.to_csv("C:\\Users\\Xian02\\数据\\Ags7_wire-cg对比.csv",index=False)
